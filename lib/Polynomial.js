...
Polynomial.prototype.getCubicRoots = function () {
    ...
    var params = []
    for (var i = this.coefs.length - 1; i >= 0; i--)
        params.push(this.coefs[i]);

    rpSolve({ Degree: this.getDegree() }, params, zeror, zeroi);

    var results = [];
    var TOLERANCE_iZero = 1e-3;
    for (var i = 0; i < zeror.length ; i++) {
        if (Math.abs(zeroi[i]) <= TOLERANCE_iZero)
            results.push(zeror[i])
    }
    return results;
};
 ...
 *
 */
Polynomial.prototype.getQuarticRoots = Polynomial.prototype.getCubicRoots;
